...
public sealed class PushCommandHandlerTests : IDisposable
{
    private readonly string _workingDirectory = Directory.CreateTempSubdirectory("nc-push-cmd-tests-").FullName;
    private readonly GitClient _git;

    public PushCommandHandlerTests()
    {
    ...
    {
        var (exitCode, writer) = await CaptureErrorAsync(() => PushCommandHandler.ExecuteAsync(_workingDirectory));
    ...
    public async Task ExecuteAsync_WithConfigSyncAndPassword_PassesLocalChecksAndFailsOnNetwork()
    {
        ...
        File.WriteAllText(Path.Combine(_workingDirectory, "nouveau.txt"), "contenu");
        _git.AddAll();
